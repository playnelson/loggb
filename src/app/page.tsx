...
export default function Home() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // 1. Stats
      const { data: items } = await supabase.from('items').select('quantity_current, quantity_min, description, code, unit');
      ...
      const { data: movements } = await supabase
        .from('movements')
        .select('*, items(description, code), employees(full_name)')
